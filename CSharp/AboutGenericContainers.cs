...
namespace DotNetKoans.CSharp
{
	public class AboutGenericContainers : Koan
	{
		...
		public void BeyondTheLimits()
		{
			...
			//Assert.Throws(typeof(IndexOutOfRangeException), delegate() { int x = list[3]; });
		}
		[Koan(12)]
		public void ConvertingToFixedSize()
		{
			List<int> list = new List<int> { 1, 2, 3 };
			//Assert.Equal(new List[], list.ToArray());
		}
		[Koan(13)]
		...
		public void AccessingDataNotAdded()
		{
			Dictionary<string, string> dictionary = new Dictionary<string, string>();
			dictionary["one"] = "uno";
			//Assert.Throws(typeof(String), delegate() { string s = dictionary["two"]; });
		}
		[Koan(22)]
